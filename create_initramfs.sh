...
[[ "$#" == "3" ]] || {
    echo "Usage: $0 <rootfs> <initramfs_dir> wb6|wb7|wb8"
    exit 1
}
...
wb8*)
    ...
    ;;
*)
    echo "Wrong board type, use wb6, wb7 or wb8"
    exit 1
    ;;
esac
...
wb7*)
    ...
    install_from_rootfs "$LIBDIR/engines-3/ateccx08.so"
    install_from_rootfs "$LIBDIR/ld-linux-armhf.so.3"
...
FROM_ROOTFS=(
    ...
    "$LIBDIR/gconv/gconv-modules.cache"
    "$LIBDIR/gconv/IBM850.so"
